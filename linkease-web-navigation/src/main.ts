import NavigationMenus from "./App.vue";
export const IconQQ: string = "data:image/png;base64,"
export const IconWechat: string = "data:image/png;base64,"
export const IconEmail: string = "data:image/png;base64,"

export const MenuObjectQQ = {
    icon: IconQQ,
    title: "QQ交流群",
    qrcode: "https://assets.koolcenter.com/linkease/linkease_qq_qrcode.png",
    email: "",
    text: ""
}
export const MenuObjectWechat = {
    icon: IconWechat,
    title: "微信扫码，立即咨询",
    qrcode: "https://assets.koolcenter.com/linkease/linkease_wechat_qrcode.png",
    email: "",
    text: ""
}
export const MenuObjectWechatConsult = {
    icon: IconWechat,
    title: "付费用户微信交流群",
    qrcode: "",
    email: "",
    text: "打开易有云APP，在“我的”-“联系我们”中查看二维码。"
}
export const MenuObjectEmail = {
    icon: IconEmail,
    title: "商务合作邮箱",
    qrcode: "",
    email: "admin@linkease.com",
    text: ""
}
const components: { [propsName: string]: any } = {
    NavigationMenus,
}
const createMenuObject = (icon: string, title: string, qrcode: string, email: string, text: string) => {
    return {
        icon,
        title,
        qrcode,
        email,
        text
    }
}
export {
    NavigationMenus,
    components,
    createMenuObject,
}
export default {
    ...components,
    NavigationMenus,
    MenuObjectWechatConsult,
    MenuObjectEmail,
    MenuObjectWechat,
    MenuObjectQQ,
    IconQQ,
    IconWechat,
    IconEmail,
    createMenuObject,
}